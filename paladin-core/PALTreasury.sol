...
contract PALTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPAL }
    ...
    address public sPAL;
    uint public sPALQueue; // Delays change to sPAL address
    ...
    /**
        @notice allow approved address to deposit an asset for PAL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PAL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PAL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PAL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPALERC20( PAL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPAL ).balanceOf( msg.sender ); // Can only borrow against sPAL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PAL
        @param _amount uint
     */
    function repayDebtWithPAL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPALERC20( PAL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PAL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PAL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PAL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PAL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PAL decimals
            value_ = _amount.mul( 10 ** IERC20( PAL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPAL ) { // 9
            sPALQueue = block.number.add( blocksNeededForQueue );
        } else return false;
