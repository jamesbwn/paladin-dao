...
contract ConvertPAL is Owned {
    ...
    bool public isConvertClose = false;

    address public PAL_ADDRESS = 0xFF7f971B8beb1f71997a1B2f06B997D7A54bC320;            // PAL:decimal 9
    ...
    function converToPAL(uint256 _amount) external {
        ...
        require( isConvertClose == false, "coverting is closed");
    ...
    function setConvertActive(bool _active) external onlyOwner {
        isConvertActive = _active;
    }

    function setConvertClose(bool _closed) external onlyOwner {
        isConvertClose = _closed;
    }
    
    function withdrawPALToken(address to) external onlyOwner {
        require(isConvertClose, "You cannot get tokens until the convert is closed.");
