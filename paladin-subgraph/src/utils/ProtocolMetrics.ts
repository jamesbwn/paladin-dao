...
import {
    ...
    PALADIN_ERC20_CONTRACT,
    SPALADIN_ERC20_CONTRACT_V1,
    STAKING_CONTRACT_V1,
    TREASURY_ADDRESS,
    USDT_ERC20_CONTRACT,
    USDC_ERC20_CONTRACT,
    ...
    PANCAKE_PALADINBUSD_PAIR,
    // STAKING_CONTRACT_V2_BLOCK,
    // STAKING_CONTRACT_V2,
    // SPALADIN_ERC20_CONTRACT_V2_BLOCK,
    // SPALADIN_ERC20_CONTRACT_V2,
    BUSD_ERC20_CONTRACT,
    // TRADERJOE_PALAVAX_PAIR,
    // TRADERJOE_PALAVAX_PAIR_BLOCK,
} from './Constants';
...
function getTotalSupply(): BigDecimal {
    let pal_contract = PalERC20.bind(Address.fromString(PALADIN_ERC20_CONTRACT))
...
function getCriculatingSupply(blockNumber: BigInt, total_supply: BigDecimal): BigDecimal {
    ...
    if (blockNumber.gt(BigInt.fromString(CIRCULATING_SUPPLY_CONTRACT_BLOCK))) {
        ...
        circ_supply = toDecimal(circulatingsupply_contract.PALCirculatingSupply(), 9)
    } else {
...
function getSPalSupply(blockNumber: BigInt): BigDecimal {
    let spal_contract_v1 = sPalERC20.bind(Address.fromString(SPALADIN_ERC20_CONTRACT_V1))
    let spal_supply = toDecimal(spal_contract_v1.circulatingSupply(), 9)

    // if (blockNumber.gt(BigInt.fromString(SPALADIN_ERC20_CONTRACT_V2_BLOCK))) {
    //     let spal_contract_v2 = sPalERC20.bind(Address.fromString(SPALADIN_ERC20_CONTRACT_V2))
...
function getPALBUSDReserves(pair: UniswapV2Pair): BigDecimal[] {
...
function getMV_RFV(blockNumber: BigInt): BigDecimal[] {
    ...
    let palbusdPair = UniswapV2Pair.bind(Address.fromString(PANCAKE_PALADINBUSD_PAIR))
    // let palavaxPair = UniswapV2Pair.bind(Address.fromString(TRADERJOE_PALAVAX_PAIR))
    ...
    let palbusdReserves = getPALBUSDReserves(palbusdPair)
    ...
    // if (blockNumber.gt(BigInt.fromString(TRADERJOE_PALAVAX_PAIR_BLOCK))) {
    ...
    //     let palavaxReserves = getPALAVAXReserves(palavaxPair)
    ...
    log.debug("Treasury PAL-BUSD RFV {}", [palbusdRFV.toString()])
    log.debug("palbusdValue {}", [palbusdValue.toString()])
    // log.debug("Treasury PAL-AVAX RFV {}", [palavaxRFV.toString()])
...
function getNextPALRebase(blockNumber: BigInt): BigDecimal {
...
function getAPY_Rebase(sPAL: BigDecimal, distributedPAL: BigDecimal): BigDecimal[] {
    let nextEpochRebase = sPAL.gt(BigDecimal.fromString('0'))
        ? distributedPAL.div(sPAL).times(BigDecimal.fromString("100"))
...
export function updateProtocolMetrics(blockNumber: BigInt, timestamp: BigInt): void {
    ...
    pm.nextDistributedPal = getNextPALRebase(blockNumber)
