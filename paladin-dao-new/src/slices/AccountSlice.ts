...
import { abi as stakingAbi } from "../abi/OlympusStakingv2.json";
...
export const loadAccountDetails = createAsyncThunk(
  ...
  async ({ networkID, provider, address }: IBaseAddressAsyncThunk) => {
    ...
    const warmUpContract = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, stakingAbi, provider);
    const warmUpInfo = await warmUpContract.warmupInfo(address);
    warmUpBalance = parseFloat(ethers.utils.formatUnits(warmUpInfo[0], "gwei"))
    warmUpExpiry = warmUpInfo[2];
    ...
    const palNFT_ADDRESS = new ethers.Contract(addresses[networkID].PALNFT_ADDRESS as string, PalNftABI, provider);
    maNftBalance = await palNFT_ADDRESS.balanceOf(address);
...
export const calculateUserBondDetails = createAsyncThunk(
  ...
  async ({ address, bond, networkID, provider }: ICalcUserBondDetailsAsyncThunk) => {
    ...
    // if (bond.isFour){
    //  bondMaturationBlock = +bondDetails.vesting / 3  + +14758277 + +(bondDetails.lastTime - 1643368396) / 3;
    // }
