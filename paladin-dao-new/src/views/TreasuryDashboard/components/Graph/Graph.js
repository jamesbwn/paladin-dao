...
export const ProtocolOwnedLiquidityGraph = () => {
  ...
  return (
    <Chart
      ...
      dataKey={["treasuryXyzDaiPOL"]}
      bulletpointColors={bulletpoints.pol}
      infoTooltipMessage={tooltipInfoMessages.pol}
      headerText="Protocol Owned Liquidity XYZ-DAI"
      expandedGraphStrokeColor={theme.palette.graphStrokeColor}
      headerSubText={`${data && trim(data[0].treasuryXyzDaiPOL, 2)}% `}
      stopColor={[["rgba(128, 204, 131, 1)", "rgba(128, 204, 131, 0)"]]}
    />
  );
};

export const XYZStakedGraph = () => {
  const theme = useTheme();
  const { data } = useTreasuryMetrics({ refetchOnMount: false });
  ...
  const staked =
    ...
    data
      .map(metric => ({
        staked: (metric.XyzsCirculatingSupply / metric.xyzCirculatingSupply) * 100,
